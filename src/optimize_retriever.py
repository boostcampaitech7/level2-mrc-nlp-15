...
retriever.sparse_embeds_bool = True
...
def objective(trial):
    ...
    retriever.sparse_embeder.sparse_embeder = BM25Plus(tokenized_docs, k1=k1, b=b, delta=delta)

    all_scores = []
    all_doc_indices = []

    for idx, query in enumerate(queries):
        # scores, contexts, doc_indices = retriever.retrieve(query, topk=20, alpha=alpha) # hybridsearch
        scores, contexts, doc_indices = retriever.retrieve(query, topk=60) # twostage-reranker
